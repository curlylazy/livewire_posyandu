            ...
            <div class="row g-4">
                @foreach ($dataRow as $row)
                    <div class="col-md-3 col-12" wire:click='selectData({{ $row }})' role="button">
                        <div class="d-flex flex-column border p-2 rounded-2">
                            <div class="d-flex">
                                <div class="fs-sm text-secondary">
                                    Include : {{ Arr::join(json_decode($row->activityinclude, true), ', ') }}
                                </div>
                            </div>
                            <h5 class="mt-2">Harga : {{ Number::format($row->harga) }}</h5 class="mt-2">
                            <h5 class="mt-0 text-justify">{{ Str::limit($row->namapackage, 50) }}</h5>
                        </div>
                    </div>
                @endforeach
            </div>

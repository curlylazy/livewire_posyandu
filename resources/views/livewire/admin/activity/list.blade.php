                    ...
                    <div class="col-md-3 col-12" wire:click='selectData({{ $row }})' role="button">
                        <div class="d-flex flex-column">
                            <div class="position-relative">
                                <img src="{{ ImageUtils::getImageThumb($row->gambaractivity) }}" class="rounded mb-2" style="height: 250px; width: 100%; object-fit: cover;"/>
                                <div class="position-absolute bottom-0 end-0 p-2">
                                    <div class="bg-light text-dark p-2 mb-2 bg-opacity-75 rounded rounded-1">
                                        <div class="text-justify fs-5 fw-bold">{{ Str::limit($row->namaactivity, 50) }}</div>
                                        <div class="text-justify fw-normal">{{ Str::limit($row->keterangansingkat, 100) }}</div>
                                    </div>
                                </div>
                            </div>
                        </div>
